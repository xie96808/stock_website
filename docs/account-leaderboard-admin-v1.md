...
2. 保留 `next_open` / `same_close`，分别排行；每个用户在同一个榜单仅占一席。主榜取最佳有效单局；另提供平均收益榜（算术平均各有效局 return_ppm）。
...
1. **密码最短长度：4 个 Unicode 码点**（不是 15）。仍保留最长 128 码点／512 UTF-8 字节、不截断、不 trim；不强制字母+数字组合，不启用弱口令黑名单（无论多么简单都可以）；前后端同一校验。放宽是产品选择，靠限流降低枚举风险。
...
- 主入口旁提供“排行榜”；用户菜单提供“我的战绩、资料设置、退出登录”。管理员菜单额外显示“管理后台”。
...
- 昵称：NFC 标准化、两端去空白后 2～16 个码点，可重名；禁止控制字符；允许含「管理员／官方／客服」等玩梗昵称（登录名仍保留 admin 等保留字）；按纯文本输出。
...
- 排行榜（`/#/leaderboard`）：榜单类型（最佳单局／平均收益）+ 两个成交模式标签、Top10、自己的名次卡、最近更新时间、规则说明（平均≠复利）、匿名或不参榜说明；游客可读。
...
榜单键：`(rule_version, dataset_version, fill_mode, metric)`。`metric` 为 `best`（最佳单局，默认）或 `average`（平均收益）。默认展示当前发布指定的规则与行情版本；旧版榜可切换查看但不与新榜合并。不加月榜／赛季系统。
...
- 每用户对该榜全部有效上榜局的 `return_ppm` 做**算术平均**（独立局均值，不是百分比点相加，也不是资金连续复利）。
- 示例：两局 +40% 与 −10%，平均为 +15%，**不是**综合 +30%。
- 排序按精确均值降序，展示时再四舍五入为 returnPpm／returnPct；并列时 finished_at（该用户有效局中最早完成时间）升序、userId 升序。

共用：

- 登录用户返回自己在完整候选集中的名次（`myRank`），而非仅 Top10 中的名次；不满足资格返回明确 reason。
...
| GET /leaderboard | fillMode（必填）、metric=`best`\|`average`（默认 best）、ruleVersion?、datasetVersion?；返回 metric、top10、total、myRank、ineligibilityReason、asOf | 公开，登录时附自己排名 |
