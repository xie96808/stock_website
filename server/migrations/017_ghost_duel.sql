-- Phase C: extend game_kind CHECK with ghost (幽灵对局).
-- Runner temporarily disables foreign_keys (see migrate.js) so the table rebuild
-- can drop/rename while child FKs stay.

CREATE TABLE game_sessions_ghost_mig (
  id TEXT PRIMARY KEY,
  ...
  game_kind TEXT NOT NULL DEFAULT 'classic'
    CHECK (game_kind IN ('classic', 'daily', 'archive_practice', 'puzzle', 'oneshot', 'survival', 'ghost')),
...
INSERT INTO game_sessions_ghost_mig (
...
ALTER TABLE game_sessions_ghost_mig RENAME TO game_sessions;
