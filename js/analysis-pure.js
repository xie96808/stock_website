// ========== ANALYSIS PURE (no DOM / no gameState) ==========
// Behavior-preserving extraction of best-points, BS score, kline analysis model, grade.
import { kbTag } from './patterns.js';

/** Null-padded MA (analysis scoring). Differs from utils.calculateMA (expanding + round). */
export function calcMANullPad(data, period) {
    const result = [];
...
export function calcSlope(arr, start, end) {
...
/**
 * @param {{ kline: object[], historyLength: number, gameDays?: number, patternTag?: (name: string) => string }} input
 * @returns {{ buys: object[], sells: object[] }}
 */
export function computeBestPoints({ kline, historyLength, gameDays = 30, patternTag = kbTag }) {
    ...
    for (let i = 0; i < gameDays; i++) {
        ...
        if (d.close > d.open && bodySize / d.close > 0.03 && bodyRatio > 0.7) {
            buyScore += 2;
            buyReasons.push({ tag: '形态', text: '出现' + patternTag('大阳线') + '，多方力量强劲，买盘推升明显' });
        }
        if (d.close < d.open && bodySize / d.close > 0.03 && bodyRatio > 0.7) {
            sellScore += 2;
            sellReasons.push({ tag: '形态', text: '出现' + patternTag('大阴线') + '，空方力量占优，卖盘压制明显' });
        }
        ...
        if (lowerShadow > bodySize * 2 && upperShadow < bodySize * 0.5 && i > 2) {
            ...
            if (prevDown) {
                buyScore += 3;
                buyReasons.push({ tag: '形态', text: '下跌后出现' + patternTag('锤子线') + '，下影线较长，空方力量衰竭' });
            }
        }
        if (upperShadow > bodySize * 2 && lowerShadow < bodySize * 0.5 && i > 2) {
            ...
            if (prevUp) {
                sellScore += 3;
                sellReasons.push({ tag: '形态', text: '上涨后出现' + patternTag('射击之星') + '，上影线较长，多方力量受阻' });
            }
        }
        ...
        if (bodyRatio < 0.1 && (d.high - d.low) > 0 && i > 2) {
            ...
            if (prevUp) {
                sellScore += 2;
                sellReasons.push({ tag: '形态', text: '上涨后出现' + patternTag('十字星') + '，多空力量达到平衡，需警惕趋势反转' });
            }
            if (prevDown) {
                buyScore += 2;
                buyReasons.push({ tag: '形态', text: '下跌后出现' + patternTag('十字星') + '，多空力量达到平衡，可能预示底部反转' });
            }
        }
        ...
        if (i > 0) {
            ...
            if (prev.close < prev.open && d.close > d.open && d.close > prev.open && d.open < prev.close) {
                buyScore += 3;
                buyReasons.push({ tag: '形态', text: patternTag('看涨吞没') + '形态（阳包阴），多方强力反攻' });
            }
            if (prev.close > prev.open && d.close < d.open && d.close < prev.open && d.open > prev.close) {
                sellScore += 3;
                sellReasons.push({ tag: '形态', text: patternTag('看跌吞没') + '形态（阴包阳），空方占据主导' });
            }
        }
    ...
    return { buys: pickSignals(buyScores), sells: pickSignals(sellScores) };
}


export function computeBSReport({
    kline,
    historyLength,
    trades,
    fillMode,
...
export function calcGrade(finalReturnPercent, bsScore) {
